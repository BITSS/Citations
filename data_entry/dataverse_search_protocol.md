Using the files `<apsr_dataverse_search>` and `<ajps_dataverse_search>`, please code the topic and type of each article. Add your initials and save the file to Box.

Use the filter function for the `<result_category>` column and select only the _empty cells

Create a new column with a title confirmed_category
