set more off
clear all
cd "C:\Users\Don\Documents\Citations\analysis"
...
line ajps_y_avg apsr_y_avg year, title("Yearly Average Availability by Journal - Data") ///
	bgcolor(white) graphregion(color(white)) ///
	ylabel(0 0.2 0.4 0.6 0.8 1)
graph export ../output/avail_time_data.eps, replace

****************************
*GRAPH CITATIONS
****************************
histogram citation, bgcolor(white) graphregion(color(white)) title("Density of Citations - Data")
...
line ajps_y_citeavg apsr_y_citeavg year, title("Yearly Average Citations by Journal - Data") ///
	bgcolor(white) graphregion(color(white))

graph export ../output/cite_time_data.eps, replace
*********************************************************
...
graph bar topic_*, stack over(ajps) legend(lab(1 "American") ///
									...
									lab(4 "Methodology") ///
									lab(5 "Theory"))
graph export ../output/topicXjournal_data.eps, replace
...
graph bar topic_*, stack over(post`X') over(ajps)  legend(lab(1 "American") ///
	...
	lab(4 "Methodology") ///
	lab(5 "Theory")) ///
	title("Article Topic by Journal Before and After `X' Policy - Data") ///
	bgcolor(white) graphregion(color(white))
...
graph bar data_type_*, stack over(post`X') over(ajps)  legend(lab(1 "Experimental") ///
	lab(2 "No Data in Article") ///
	lab(3 "Observational") ///
	lab(4 "Simulations")) ///
	title("Data Type by Journal Before and After `X' Policy - Data") ///
	bgcolor(white) graphregion(color(white))
graph export ../output/typeXjournalXpost`X'_data.eps, replace
}
...
histogram top_rank, title("Top US News Ranking of Articles - Data") ///
	bgcolor(white) graphregion(color(white)) ///
	note("*Rank of 125 implies no author at top-100 ranked institution")
graph export ../output/histo_authrank_data.eps, replace
...
graph bar top1 top5 top20 top50 top100 unranked, stack over(post`X') over(ajps)  legend(lab(1 "Top 1") ///
	...
	lab(5 "Top 100") ///
	lab(6 "Unranked")) ///
	title("Institution Rankings by Journal Before and After `X' Policy - Data") ///
	bgcolor(white) graphregion(color(white))
graph export ../output/rankXjournalXpost`X'_data.eps, replace
}
