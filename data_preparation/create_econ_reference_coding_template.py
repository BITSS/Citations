...
for ix, df in enumerate(pd.read_csv(input_file, chunksize=1000)):
    ...
    if ix == 0:
        df.to_csv(output_file, mode='w', encoding='utf-8', index=None, columns=output_columns)
    else:
        df.to_csv(output_file, mode='a', encoding='utf-8', index=None, header=None, columns=output_columns)